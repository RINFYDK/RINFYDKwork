...
PACKAGECONFIG ??= "${@bb.utils.contains('TARGET_ARCH', 'powerpc', 'noatomics', '', d)} \
                   ...
                   ${@bb.utils.contains('TARGET_ARCH', 'armeb', 'noatomics', '', d)} \
...
do_install_append() {
    if ${@bb.utils.contains('DISTRO_FEATURES', 'systemd', 'true', 'false', d)}; then
        ...
        echo "d ${localstatedir}/${BPN} 0750 squid squid -" >> ${D}${sysconfdir}/tmpfiles.d/${BPN}.conf
    fi

    install -d ${D}/${localstatedir}/${BPN}
    install -m 0755 ${WORKDIR}/squid.conf ${D}/${localstatedir}/${BPN}/${BPN}.conf
...
CONFFILES_${PN} =" ${localstatedir}/${BPN}/${BPN}.conf "
