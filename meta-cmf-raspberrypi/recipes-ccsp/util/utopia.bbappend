...
SRC_URI_append = " \
    file://0001-fix-lan-handler-for-rpi.patch \
    file://0002-fix-ulog-dependency-for-firewall.patch \
    file://0003-fix-udhcpc-command-for-RPi.patch \
    file://0004-postd_services_registration.patch \
    file://0005-firewall_parental_control.patch \
...
do_install_append() {
    ...
    install -D -m 644 ${WORKDIR}/git/source/scripts/init/syslog_conf/syslog.conf_default ${D}/fss/gw/${sysconfdir}/syslog.conf.${BPN}
    ...
    install -m 755 ${WORKDIR}/git/source/scripts/init/service.d/service_dhcpv6_client_arm.sh ${D}${sysconfdir}/utopia/service.d/service_dhcpv6_client.sh
    install -m 755 ${WORKDIR}/git/source/scripts/init/system/need_wifi_default.sh ${D}${sysconfdir}/utopia/
